<div align="center">

<img src="../assets/logo-animated.svg" width="120" height="120" alt="Intelligent Machines logo" />

# Intelligent Machines

**We solve mission-critical business problems faster than anyone else.**

[![Website](https://img.shields.io/badge/Website-intelmachines.ai-1264FF?style=flat-square&logo=googlechrome&logoColor=white)](https://intelmachines.ai)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Intelligent%20Machines-1264FF?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/company/intelligent-machines-ltd)
[![Email](https://img.shields.io/badge/Email-now%40intelmachin.es-1264FF?style=flat-square&logo=gmail&logoColor=white)](mailto:now@intelmachin.es)

</div>

## About

From AI-powered enterprise solutions to custom-built integrations, we transform
complexity into clarity and empower businesses with cutting-edge technology.
Since 2018 we've helped Bangladesh's largest distribution businesses run smarter with production-grade AI.

<div align="center">

| 59 | 16 | 6 |
|:---:|:---:|:---:|
| production systems | enterprise clients | countries |

</div>

## What we build

| Product | What it does |
|---|---|
| **Biponon.ai** | Trade marketing that runs on facts |
| **Bitoron.ai** | Leak-proof disbursal & claims |
| **Challan.ai** | The reliable bridge to government rails |
| **Fordo.ai** | Handwriting to clean data, at scale |
| **Infinity Retail** | Territory plans that actually happen |
| **Nimontron.ai** | Merchant lifecycle without the chaos |
| **Dharapat** | CIB analysis that defends every decision |

## Focus areas

![AI/ML](https://img.shields.io/badge/AI%2FML-1264FF?style=flat-square)
![OCR](https://img.shields.io/badge/OCR-1264FF?style=flat-square)
![Fraud Detection](https://img.shields.io/badge/Fraud%20Detection-1264FF?style=flat-square)
![Data Pipelines](https://img.shields.io/badge/Data%20Pipelines-1264FF?style=flat-square)
![Real-time Dashboards](https://img.shields.io/badge/Real--time%20Dashboards-1264FF?style=flat-square)

## We're hiring

Join a team solving the hardest enterprise AI challenges on the planet. We move fast, ship real products, and our work changes how the world's largest companies operate.
[See open roles →](https://intelmachines.ai/career)

## Get in touch

- **Website:** [intelmachines.ai](https://intelmachines.ai)
- **Email:** [now@intelmachin.es](mailto:now@intelmachin.es)
- **Phone:** +8801819218449
- **LinkedIn:** [Intelligent Machines](https://www.linkedin.com/company/intelligent-machines-ltd)
- **Address:** Momen Shah,House #68, Road #17, Block E, Level 10, Banani, Dhaka
